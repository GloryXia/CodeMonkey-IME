...
# ============================================================
# Hybrid IME — 卸载脚本
# 从 macOS Rime (Squirrel) 用户目录移除 Hybrid IME
# ============================================================
...
echo -e "${BLUE}╔══════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║     Hybrid IME — 混合输入法 卸载程序     ║${NC}"
echo -e "${BLUE}╚══════════════════════════════════════════╝${NC}"
echo ""

read -p "确认卸载 Hybrid IME？(y/n) " -n 1 -r
echo ""
if [[ ! $REPLY =~ ^[Yy]$ ]]; then
...
if [ -f "$RIME_DIR/rime.lua" ]; then
    ...
    sed -i '' '/Hybrid IME/d' "$RIME_DIR/rime.lua" 2>/dev/null || true
...
echo ""
echo -e "${GREEN}Hybrid IME 已成功卸载${NC}"
